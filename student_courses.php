			...
			if($result->num_rows > 0) {
				while($row = $result ->fetch_assoc()) {
					if($row['start_date'] > date('yy-m-d')) { ?>
						...
						<div class="form-group border bg-dark text-white">
							...
							<br>
							<textarea name="summary" disabled="true" id="summary" class="border-0 float-left summary bg-dark text-white" style="display: none;" >
								<?php echo $row['summary'] ?>
							</textarea>
							<br>
							<p class="m-0">Start date:</p>
							<input type="date" name="start_date" disabled="true" id="start_date" class="form-control border-0 bg-dark text-white" value="<?php echo $row['start_date'] ?>">
							...
							<br>
							<input type="button" class="btn btn-outline-primary font-weight-bold mb-3" name="enroll_id" id="<?php echo $row['id']; ?>" value="ENROLL NOW" onclick = "return on_click(this.id)" >
						</div>
						<br>
					<?php } else { ?>
